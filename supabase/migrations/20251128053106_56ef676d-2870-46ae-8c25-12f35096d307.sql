-- Create GIN indexes for Full-Text Search on all pipeline chunks tables
CREATE INDEX IF NOT EXISTS idx_pipeline_a_hybrid_chunks_content_fts 
ON pipeline_a_hybrid_chunks_raw 
USING GIN (to_tsvector('english', content));

CREATE INDEX IF NOT EXISTS idx_pipeline_a_chunks_content_fts 
ON pipeline_a_chunks_raw 
USING GIN (to_tsvector('english', content));

CREATE INDEX IF NOT EXISTS idx_pipeline_b_chunks_content_fts 
ON pipeline_b_chunks_raw 
USING GIN (to_tsvector('english', content));

-- Create RPC function for keyword search fallback using PostgreSQL FTS
